...
@Component({
  ...
  providers: [AuthService, UserService, ResourceService, Router, MatSnackBar, RouterLink, RouterLinkActive, FormsModule, ReactiveFormsModule],
...
export class Schedule {
  ...
  validateHasSchedule() {
    this._resourceService.getResourceByUserId(this.user._id).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        this._snackBar.open("Não foi possível validar se usuário já possui horários.", "", { duration: 2000 });
      },
    })
  }
